...
function namedFilter(name)

if isequal(name[1:min(5,length(name))], "binom")
  #kernel = sqrt(2) * binomialFilter(str2num(name(6:size(name,2))));
  error("Binomial filter not implemented...")
elseif isequal(name,"qmf5")
  kernel = [-0.076103 0.3535534 0.8593118 0.3535534 -0.076103]
elseif isequal(name,"qmf9")
  kernel = [0.02807382 -0.060944743 -0.073386624 0.41472545 0.7973934 0.41472545 -0.073386624 -0.060944743 0.02807382]
elseif isequal(name,"qmf13")
  kernel = [-0.014556438 0.021651438 0.039045125 -0.09800052 -0.057827797 0.42995453 0.7737113 0.42995453 -0.057827797 -0.09800052 0.039045125 0.021651438 -0.014556438]
elseif isequal(name,"qmf8")
  kernel = sqrt(2) * [0.00938715 -0.07065183 0.06942827 0.489980 0.4899808 0.06942827 -0.07065183 0.00938715 ]
elseif isequal(name,"qmf12")
  kernel = sqrt(2) * [-0.003809699 0.01885659 -0.002710326 -0.08469594 0.08846992 0.4843894 0.4843894 0.08846992 -0.08469594 -0.002710326 0.01885659 -0.003809699 ]
elseif isequal(name,"qmf16")
  kernel = sqrt(2) * [0.001050167 -0.005054526 -0.002589756 0.0276414 -0.009666376 -0.09039223 0.09779817 0.4810284 0.4810284 0.09779817 -0.09039223 -0.009666376 0.0276414 -0.002589756 -0.005054526 0.001050167 ]
elseif isequal(name,"haar")
  kernel = [1 1] / sqrt(2)
elseif isequal(name,"daub2")
  kernel = [0.482962913145 0.836516303738 0.224143868042 -0.129409522551]
elseif isequal(name,"daub3")
  kernel = [0.332670552950 0.806891509311 0.459877502118 -0.135011020010
	-0.085441273882  0.035226291882]
elseif isequal(name,"daub4")
  kernel = [0.230377813309 0.714846570553 0.630880767930 -0.027983769417 -0.187034811719 0.030841381836 0.032883011667 -0.010597401785]
elseif isequal(name,"gauss5")  # for backward-compatibility
  kernel = sqrt(2) * [0.0625 0.25 0.375 0.25 0.0625]
elseif isequal(name,"gauss3")  # for backward-compatibility
  kernel = sqrt(2) * [0.25 0.5 0.25]
else
  error(string("Bad filter name: ",name))
end
